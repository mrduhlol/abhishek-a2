...
function SystemVisual() {
  ...
  return (
    ...
    >
      ...
      />
      {/* orbit diagram */}
      <svg viewBox="0 0 400 340" className="relative block h-auto w-full" role="img" aria-label="Abstract system diagram">
        <circle cx="200" cy="165" r="96" fill="none" stroke="rgba(255,255,255,0.10)" />
        <circle cx="200" cy="165" r="58" fill="none" stroke="rgba(255,255,255,0.08)" />
        ...
        <circle cx="296" cy="165" r="4" fill="rgba(255,255,255,0.7)" />
        <line x1="200" y1="165" x2="200" y2="69" stroke="rgba(255,255,255,0.14)" />
        <line x1="200" y1="165" x2="296" y2="165" stroke="rgba(255,255,255,0.14)" />
        <text x="212" y="72" fill="rgba(255,255,255,0.45)" fontSize="10" fontFamily="monospace">client</text>
        <text x="212" y="168" fill="rgba(255,255,255,0.45)" fontSize="10" fontFamily="monospace">core</text>
        <text x="252" y="158" fill="rgba(255,255,255,0.45)" fontSize="10" fontFamily="monospace">edge</text>
      </svg>
      {/* floating fragments */}
      <div className="pointer-events-none absolute left-4 top-4 rounded-md border border-white/10 bg-[#0B0D12]/90 px-3 py-2 font-mono text-[11px] text-white/60">
        <span className="text-[#5B8CFF]">$</span> whoami<span className="animate-pulse">_</span>
      </div>
      <div className="pointer-events-none absolute bottom-4 right-4 rounded-md border border-white/10 bg-[#0B0D12]/90 px-3 py-2 font-mono text-[11px] text-white/60">
      ...
      <div className="mx-auto max-w-6xl px-5 py-24 md:px-8 md:py-36">
        ...
        <div className="grid items-start gap-12 md:grid-cols-12 md:gap-10">
          <div className="md:col-span-7">
            ...
            <Reveal delay={0.1}>
              <p className="mt-5 max-w-xl leading-relaxed text-[#9CA3AF]">
            ...
            <Reveal delay={0.16}>
              <ul className="mt-8 flex flex-wrap gap-2.5" aria-label="Interests">
                {ABOUT_INTERESTS.map((t) => (
                  <li
                    key={t}
                    className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-[13px] text-white/75 transition-colors duration-300 hover:border-[#5B8CFF]/40 hover:text-white"
                  >
                    {t}
                  </li>
                ))}
            ...
            <Reveal delay={0.2}>
              <dl className="mt-10 grid max-w-xl grid-cols-2 gap-px overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.08] sm:grid-cols-3">
                {[
                  ["Focus", "Build + Break"],
                  ["Base", "India"],
                  ["Mode", "Open to collab"],
                ].map(([k, v]) => (
                  <div key={k} className="bg-[#0B0D12] px-5 py-4">
                    <dt className="text-[11px] font-semibold tracking-[0.2em] text-white/35 uppercase">{k}</dt>
